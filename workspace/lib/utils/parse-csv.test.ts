...
test("ヘッダー行のみの場合は空の配列を返す", async () => {
  const result = await parseCsv("name,age", ["name", "age"])
  expect(result).toEqual([])
})
...
Bob,25`
  ...
  const result = await parseCsv(csv, ["name", "age"])
  expect(result).toEqual([
    { name: "Alice", age: "30" },
    { name: "Bob", age: "25" },
  ])
...
"Bob","He is ""awesome"""`
  ...
  const result = await parseCsv(csv, ["name", "description"])
  expect(result).toEqual([
...
Bob,apple`
  ...
  const result = await parseCsv(csv, ["name", "items"])
  expect(result).toEqual([
...
test("単純なCSVを正しくパースする", async () => {
  ...
  const result = await parseCsv(csv, ["id", "name", "age"])
...
test("ダブルクォートで囲まれた値を正しく処理する", async () => {
  ...
  const result = await parseCsv(csv, ["id", "name", "description"])
...
test("指定されたキーのみを抽出する", async () => {
  ...
  const result = await parseCsv(csv, ["id", "name"])
